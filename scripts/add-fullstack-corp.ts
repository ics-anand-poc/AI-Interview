/**
 * Merge "Full Stack Developer.xlsx" into live Corp Pool.
 * Keeps everyone already in the pool. New people get today's upload batch.
 *
 * Usage:
 *   npx tsx scripts/add-fullstack-corp.ts
 */
...
const CORP_PATH = "C:/Users/Aryan/Downloads/Full Stack Developer.xlsx";
const CORP_SOURCE = "Full Stack Developer.xlsx";
...
async function parsePeople() {
  ...
  sheet.eachRow((row) => {
    ...
    const email = (idxs.email >= 0 ? vals[idxs.email] : "") || prev?.email || "";
    byId.set(id, {
      employee_id: id,
...
async function main() {
  ...
  if (!incoming.length) throw new Error("No people parsed from Full Stack Developer.xlsx");
  ...
  for (const person of incoming) {
    ...
    const fileEmail = person.email && person.email.includes("@") ? person.email : "";
    const keepEmail =
      fileEmail ||
