/**
 * Add Senior Full Stack Developer SLED and Java-focused Core Product JDs.
 * Does not delete or replace existing requirements.
 *
 * Usage:
 *   npx tsx scripts/add-sled-java-jds.ts
 */
...
const REQUIREMENTS = [
  {
    brId: "88004BR",
    file: "Senior Full Stack Developer - SLED.txt",
  },
  {
    brId: "88005BR",
    file: "Senior Full Stack Developer - Core Product - Java Focused.txt",
  },
];
...
async function main() {
  ...
  for (const row of rows) {
    const idx = local.findIndex(
      (j) => j.id === row.id || String(j.fileName || "").includes(row.file_name.split(" | ")[1])
    );
