...
/** Structural validation for one generated question object — item 10's "predictable,
 *  validated JSON structures", applied at the call site since the shape is specific to
 *  question generation (ai-providers/generateAIJson only validates "is this JSON"). */
function isValidQuestionObject(item: any): boolean {
  return (
    !!item &&
    typeof item === "object" &&
    typeof item.question === "string" &&
    item.question.trim().length > 0 &&
    ALLOWED_QUESTION_DIFFICULTIES.has(item.difficulty) &&
    typeof item.skill === "string" &&
    Array.isArray(item.expectedPoints)
  );
}

// Accepts a small model returning slightly more/fewer items than requested, or a few
// individually malformed items among otherwise-good ones, instead of hard-failing the
// whole batch over one bad item — filters to structurally valid items first, then
// truncates if over, accepts if reasonably close, only rejects if what's left is empty
// or far short of what was asked for.
function acceptGenerated(arr: any, requested: number): any[] {
  ...
  const valid = arr.filter(isValidQuestionObject);
  if (valid.length < Math.ceil(requested * 0.6)) {
    throw new Error(
      `Expected ~${requested} valid question objects, got only ${valid.length} usable of ${arr.length} parsed`
    );
  }
  return valid.length > requested ? valid.slice(0, requested) : valid;
}
...
`;
      // generateAIJson retries across providers on a malformed/unparseable response,
      // not just on network failure — see its doc comment in ai-providers/index.ts.
      overallFeedback = await generateAIJson(prompt);
