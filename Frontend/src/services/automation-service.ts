...
/**
 * Ensures the folders /docs/Resumes, /docs/BR-JD, and /docs/Corp Pool exist (local dev),
 * or the Supabase "docs-ingest" bucket exists (Vercel/cloud — local dirs aren't writable
 * there, so mkdir on them would throw EROFS). Delegates to docs-storage.ts's own
 * cloud/local branching instead of duplicating it here.
 */
export async function ensureDocsDirectories() {
  await ensureDocsStorage();
  await mkdir(getUploadsRoot(), { recursive: true }).catch(() => {});
}
...
`;
    ...
    // generateAIJson retries across providers on a malformed/unparseable
    // response, not just a network failure — see ai-providers/index.ts.
...
export async function refreshRequirements(): Promise<{ success: boolean; processedBRs: number; convertedJDs: number }> {
  ...
  for (const file of actualJdFiles) {
    ...
    try {
      ...
      // Save converted BR spreadsheet back to BR folder (Supabase docs-ingest bucket in
      // cloud deployments, local docs/BR in dev)
      const outputBrName = `${base}_BR.xlsx`;
      const finalBuffer = await workbook.xlsx.writeBuffer();
      await writeDocFile("BR", outputBrName, Buffer.from(finalBuffer as any));
