...
import { readDocFileBuffer } from "@/lib/docs-storage";
...
// Sourced via readDocFileBuffer (docs-storage.ts) so these resolve against the Supabase
// "docs-ingest" bucket on Vercel/cloud deployments instead of a bundled local path that
// doesn't exist once deployed (was previously read via a hardcoded process.cwd() path,
// which is why this crashed with "File not found" in production/serverless).
const MAPPING_FILENAME = "Resource_Question_Mapping.xlsx";
const CREDENTIALS_FILENAME = "Employee_User_Credentials.xlsx";
...
async function loadResourceQuestionMappingFromExcel(): Promise<PortalProfileRow[]> {
  ...
  const buffer = await readDocFileBuffer("Portal Mapping", MAPPING_FILENAME);
...
export async function loadEmployeeCredentialsRoster(): Promise<PortalProfileRow[]> {
  ...
  try {
    const workbook = new ExcelJS.Workbook();
    const buffer = await readDocFileBuffer("Portal Mapping", CREDENTIALS_FILENAME);
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.worksheets[0];
    if (!sheet) return [];
