...
export default function CandidatePortal() {
    ...
    try {
      ...
      if (savedStr) {
        ...
        if (saved.currentStep !== 'setup') {
          // Restart timer
          timerStartRef.current = Date.now() - (TOTAL_SECONDS - saved.secondsLeft) * 1000;
  ...
  const finalizeInterview = () => {
    ...
    fetch(`${BACKEND_URL}/api/interview/${resumeId}/conclude`, { method: 'POST' }).catch(e => {
  ...
  const triggerProctorEvent = useCallback(async (
    ...
    setWarningCount(prevCount => {
      ...
      fetch(`${BACKEND_URL}/api/interview/${resumeId}/proctor_violation`, {
        method: 'POST',
  ...
  // Proctor State checks — faceproj (YOLO nano for presence/count/phone, MTCNN for gaze)
  useEffect(() => {
    ...
    intervalId = setInterval(async () => {
      ...
      inFlight = true;
      try {
        const res = await fetch(`${BACKEND_URL}/api/interview/${resumeId}/monitor`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ frame }),
        });
        if (!res.ok) return;
        const { state: detectedState } = (await res.json()) as { state: typeof lastState };
        ...
        if (smoothedState !== lastState) {
          ...
          if (!hasIncrementedWarning) {
            if (lastState === 'none' && duration >= faceMissingDurationThreshold) {
              ...
              hasIncrementedWarning = true;
            } else if (lastState === 'down' && duration >= lookAwayDurationThreshold) {
              const secondsSinceKeystroke = (now - lastKeystrokeAt) / 1000;
              if (secondsSinceKeystroke > 5) {
                setWarningCount(prev => {
                  const next = prev + 1;
                  warningCountRef.current = next;
                  return next;
            ...
            } else if (['left', 'right', 'up'].includes(lastState) && duration >= lookAwayDurationThreshold) {
              setWarningCount(prev => {
                const next = prev + 1;
                warningCountRef.current = next;
                return next;
              ...
              hasIncrementedWarning = true;
            } else if (lastState === 'multiple' && duration >= 2.0) {
              setWarningCount(prev => {
                const next = prev + 1;
                warningCountRef.current = next;
                return next;
              ...
              hasIncrementedWarning = true;
            } else if (lastState === 'phone' && duration >= 2.0) {
              setWarningCount(prev => {
                const next = prev + 1;
                warningCountRef.current = next;
                return next;
  ...
  }, [currentStep, interviewEnded, proctorState, triggerProctorEvent, resumeId, questions, currentIndex]);
  ...
  const handleVerifyIdentity = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/interview/${resumeId}/verify_id`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const startTimer = () => {
    timerStartRef.current = Date.now() - (TOTAL_SECONDS - secondsLeft) * 1000;
    timerIdRef.current = window.setInterval(() => {
      const elapsed = Math.floor((Date.now() - (timerStartRef.current as number)) / 1000);
      const remaining = Math.max(0, TOTAL_SECONDS - elapsed);
      setSecondsLeft(remaining);
