...
export async function extractResumeStructureWithAI(text: string): Promise<ParsedResume> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      // response (not just a network failure) and does its own, more robust
      // bracket-balanced extraction — see ai-providers/index.ts.
      const json = await generateAIJson(prompt);
