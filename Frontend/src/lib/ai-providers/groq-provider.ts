    ...
    try {
      res = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.2,
          // Reasoning-capable Groq models (e.g. the openai/gpt-oss-* family used by
          // GROQ_MODEL's default) can otherwise interleave chain-of-thought text into
          // `message.content` ahead of the actual answer, which breaks naive
          // JSON-boundary parsing. "hidden" keeps content to the final answer only.
          // Ignored by Groq for models that don't support reasoning — safe either way.
          // Not verified against the live Groq API from this environment (no network
          // access to api.groq.com here); confirm the response shape once you can hit
          // the real API, and drop this field if it turns out to error for your model.
