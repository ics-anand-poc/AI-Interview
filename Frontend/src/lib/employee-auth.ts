...
function getAuthSecret(): string {
  return process.env.EMPLOYEE_AUTH_SECRET || AUTH_SECRET || "dev-employee-auth-secret";
}
...
export function verifyToken(token: string) {
  ...
  const secrets = Array.from(new Set([
    ...
    AUTH_SECRET,
    "dev-employee-auth-secret"
  ].filter(Boolean))) as string[];

  let isValid = false;
  for (const sec of secrets) {
    const expected = crypto.createHmac("sha256", sec).update(payload).digest("hex");
    if (signature.length === expected.length && crypto.timingSafeEqual(Buffer.from(signature, "utf8"), Buffer.from(expected, "utf8"))) {
      isValid = true;
      break;
    }
  }
